...
<form action="/myServlet" method="post">
  ...
  <font size="2" color="red"><%=request.getAttribute("errorId") != null?request.getAttribute("errorId"):request.getAttribute("nullErrors")%></font></label>
  <p> имя: <label><input type="text" name="firstName"
                         placeholder="name"
                         value="<%=request.getAttribute("fName")%>"></label></p>
  <font size="2" color="red"><%=request.getAttribute("errorsFName") != null?request.getAttribute("errorsFName"):request.getAttribute("nullErrors")%></font></label>
  <p> фамилия: <label><input type="text" name="secondName"
                             placeholder="second name"
                             value="<%=request.getAttribute("sName")%>"></label></p>
  <font size="2" color="red"><%=request.getAttribute("errorsSName") != null?request.getAttribute("errorsSName"):request.getAttribute("nullErrors")%></font>
  <p> дата рождения: <label><input type="text" name="birthDate"
                                   placeholder="yyyy-mm-dd"
                                   value="<%=request.getAttribute("birthDate")%>"></label></p>
  <font size="2" color="red"><%=request.getAttribute("errorsBirthDate")!=null?request.getAttribute("errorsBirthDate"):request.getAttribute("nullErrors")%></font>

  <p> логин: <label><input type="text" name="login"
                           placeholder="login"
                           value="<%=request.getAttribute("login")%>"></label></p>
  <font size="2" color="red"><%=request.getAttribute("errorsLogin")!=null?request.getAttribute("errorsLogin"):request.getAttribute("nullErrors")%></font>

  <p> пароль: <label><input type="password" name="password"
                            placeholder="password"></label></p>
  <font size="2" color="red"><%=request.getAttribute("errorsPassword")!=null?request.getAttribute("errorsPassword"):request.getAttribute("nullErrors")%></font>

  <p> e-mail: <label><input type="email" name="email"
                            placeholder="e-mail"
                            value="<%=request.getAttribute("eMail")%>"></label></p>
  <font size="2" color="red"><%=request.getAttribute("errorsEmail")!=null?request.getAttribute("errorsEmail"):request.getAttribute("nullErrors")%></font>
